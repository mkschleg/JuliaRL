...
Details a learning update for a reinforcement learning algorithm.
"""
...
function update!(model, opt, lu::TD, ρ::Array{T}, s_t, s_tp1, r::Array{T}, γ::Array{T}, terminal; corr_term=1.0f0) where {T <: Number}
    v_t = model.(s_t)
    v_tp1 = model.(s_tp1)
    loss = offpolicy_tdloss(ρ.*corr_term, v_t, r, γ, Flux.data.(v_tp1))
    grads = Flux.gradient(()->loss, params(model))
